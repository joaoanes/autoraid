defmodule Autoraid.Stats do
  ...
  def handle_info(:broadcast_stats, %{available_bosses: available_bosses, app_supervisor: nil, supervisor: supervisor, interval: interval} = state) do
  ...
  def handle_info(:broadcast_stats, %{available_bosses: available_bosses, app_supervisor: app_supervisor, interval: interval} = state) do
  ...
  def handle_info(:broadcast_stats, %{available_bosses: available_bosses, registry_pid: r_pid, queues_pid: q_pid, interval: interval} = state) do
    broadcast(available_bosses, r_pid, q_pid)
  ...
  def broadcast(available_bosses, r_pid, q_pid) do
    stats = Enum.map(available_bosses, fn boss_name ->
    ...
    Registry.Autoraid.Stats
        |> Registry.dispatch(
          ...
          fn registrations ->
            registrations
            |> Enum.each(fn {pid, _user} -> Process.send(pid, stats |> Jason.encode!, []) end)
          end
        )
  end
